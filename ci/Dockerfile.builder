...
# sfor_hash
RUN git clone https://github.com/sforpay/sfor_hash
RUN cd sfor_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} sfor
RUN useradd -u ${USER_ID} -g sfor -s /bin/bash -m -d /sfor sfor
...
RUN mkdir /sfor-src && \
  ...
  chown $USER_ID:$GROUP_ID /sfor-src && \

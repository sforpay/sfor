...
// which are part of the Sfor repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="sfor-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /sfor-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "sforpay-sfor/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from sforpay-sfor/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'sforpay-sfor/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from sforpay-sfor/develop"
            }
          }

          if (hasCache) {
            sh "cd /sfor-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /sfor-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /sfor-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /sfor-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /sfor-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /sfor-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /sfor-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }

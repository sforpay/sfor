...
const char * const BITCOIN_CONF_FILENAME = "sfor.conf";
const char * const BITCOIN_PID_FILENAME = "sford.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "sfor" is a composite category enabling all Sfor-related debug output
                if(ptrCategory->count(std::string("sfor"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SforCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SforCore
    // Mac: ~/Library/Application Support/SforCore
    // Unix: ~/.sforcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SforCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SforCore";
#else
    // Unix
    return pathRet / ".sforcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty sfor.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override sfor.conf

...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all Sfor related functionality
...
/*
    ...
    allow endless transaction that would bloat Sfor and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all Sfor specific functionality
